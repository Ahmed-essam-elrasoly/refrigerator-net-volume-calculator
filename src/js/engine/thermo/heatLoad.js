// heatLoad.js – exact replica of Excel SIZE sheet
...
function kUrethane(t) {
  if (!kUrethaneCache.has(t)) kUrethaneCache.set(t, calcK(t, PC.insulation.urethane));
  return kUrethaneCache.get(t);
}

/**
 * @param {object} geom
 * @param {object} temps - { T0, TF, TR, T2, TC, PR }
 * @param {object} electrical
 * @param {object} condenserRises - { side: number, back: number }
 */
export function calcHeatLoads(geom, temps, electrical, condenserRises = { side: 0, back: 0 }) {
  const {
  ...
  // Surface temperatures that depend on condenser heat
  const T_side  = T0 + condenserRises.side * PR;
  const T_back  = T0 + condenserRises.back * PR;
  // Refrigerator back wall temperature: Excel (F3-H4)*F1+H4 where Comp temp = 50*PR + T0
  //const T_rback = T0 + 50 * PR * PR;
  ...
  // Partition losses (exact Excel formulas)
  ...
  // Refrigerator base heat transfers
  ...
  // Refrigerator partition loss
  const DPCON_R = (0.0546 * (TC - TF) - 0.0491 * (T0 - TF)) * PR * (Hr * 2 + W) / 1000;
  QR += DPCON_R;

  // ---------- Evaporator back ----------
  const A_evaBack = (W - (tFleft + tFright) / 2) * (Hf - (tFtop + tFbottom) / 2) / 1e6;
  const QEV = kUrethane(tEvaBack) * A_evaBack * (T_back - T2);

  // Fan / defrost loads
  const fanLoad     = electrical.pwbOn_W * PC.conversion.wattToKcalPerH;
