(() => {
  ...
  function solveInner(TC, geom, compParams, refrigerant, subcool, fixedTemps, fan, electrical, condenserConfig, TE, freezerPos, innerOpts = {}) {
    ...
    const F = ([T2, PR]) => {
      ...
      F2 = loads2.QF + loads2.QR + loads2.QEV - comp2.QCompressor * PR;
...
Outer ${iter}, TC=${TC.toFixed(2)}`);
  ...
  function airSpeed(fanParam, evap) {
    ...
    const fanAirflow_m3h = Math.PI * (fanDiam / 2) ** 2 * fanThick * fanRPM / 1e6 * 3600;
  ...
  function handleRun() {
    ...
    const freezerPosition = nComps === 1 ? "top" : (
      // single compartment always "top" (no other)
    ...
    <table class="thermo-results-table">
      ...
      <tbody>
        ...
        ${res.evapDetails ? `
          ...
          <tr><td>Heat transfer coeff \u03B1</td><td>${fmt(res.evapDetails.alpha, 2)} kcal/h\xB7m\xB2\xB7\xB0C</td></tr>
