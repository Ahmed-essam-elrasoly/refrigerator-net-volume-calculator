// tests/debug_final_v8.mjs – correct outside temps for R BACK/BOTTOM1/BOTTOM2, correct area formulas
...
function heatLoads(geom, temps, elec, rises, fanFlow, fanW) {
  ...
  const T_side = T0 + rises.side;                // cabinet side (condenser)
  const T_back = T0 + rises.back;                // cabinet back (condenser) – for evap only
  const T_comp = 50*PR + T0;                     // compressor temperature
  const T_wallBack = (T_comp - T0)*PR + T0;      // R BACK / BOTTOM1/2 outside temp
  ...
  QF += (0.1219*(TC-TF)*PR + 0.1219*((0.1984*T0+0.1219*TF)/(0.1984+0.1219)-TF)*(1-PR)) * (W-tFleft-tFright)/1000;
  QF += (0.0791*(TC-TF)-0.072*(T0-TF))*PR * (Hf*2+W)/1000;

  // ── Refrigerator areas (corrected: use tRbottom2, not tRbottom1) ──
  const ARtop    = (W-(tRleft+tRright)/2)*(D-tRback/2)/1e6;
  const ARleft   = ((Hr-(tRbottom2+tRbottom3)/2)*(D-tRback/2) - (Db1+Db2)*Hb/2)/1e6;
  const ARback   = (Hr-(tRbottom2+tRbottom3)/2-Hb) * (W-(tRleft+tRright)/2)/1e6;
  ...
  let QR = kInterior(tRtop,TF,TR)*ARtop*(TF-TR)
         ...
         + kExterior(tRback,TR,T_wallBack)*ARback*(T_wallBack-TR)        // ← T_wallBack
         + kExterior(tRbottom1,TR,T_wallBack)*ARbottom1*(T_wallBack-TR)  // ← T_wallBack
         + kExterior(tRbottom2,TR,T_wallBack)*ARbottom2*(T_wallBack-TR)  // ← T_wallBack
  ...
  const QEV_cond = kExterior(tEvaBack,T2,T_back)*A_evaBack*(T_back-T2); // evap back uses T_back (condenser back)
...
function compressorStateCorrected(TC, TE, refrig, compParams, subcool) {
  const rf = getRefrigerantFunctions(refrig);
  const Pe = rf.satPressure(TE);
  const v_suc = rf.specificVolume(compParams.T_suction, Pe);
  ...
  const Kw = compParams.powerKw.a + compParams.powerKw.b*compParams.rpm + compParams.powerKw.c*compParams.rpm*compParams.rpm;
  const power = base * Kw * (compParams.rpm/compParams.rpm0);
  return { etaV, massFlow: mdot, coolingCapacity: cooling, inputPower: power, h_evap_out, h_liquid };
}

// ── Newton / inner / outer (unchanged) ────────────────────────────────
function newton2(F,x0,dx,tol,maxI){
  let x=[x0[0],x0[1]];
  for(let i=0;i<maxI;i++){
    const f=F(x); if(Math.max(Math.abs(f[0]),Math.abs(f[1]))<=tol) return {x,converged:true,iterations:i+1};
    const J=[[0,0],[0,0]];
    for(let j=0;j<2;j++){const xp=[x[0],x[1]];xp[j]+=dx;const fp=F(xp);J[0][j]=(fp[0]-f[0])/dx;J[1][j]=(fp[1]-f[1])/dx;}
    const det=J[0][0]*J[1][1]-J[0][1]*J[1][0]; if(Math.abs(det)<1e-12) return {x,converged:false,iterations:i+1,error:'Singular'};
    x[0]=Math.max(-80,Math.min(20,x[0]+(-f[0]*J[1][1]+f[1]*J[0][1])/det));
    x[1]=Math.max(0.001,Math.min(0.999,x[1]+(J[0][0]*(-f[1])+J[1][0]*f[0])/det));
  }
  return {x,converged:false,iterations:maxI,error:'Max iters'};
}
...
function solveInner(TC,geom,compParams,refrig,subcool,fixedTemps,fan,elec,condConfig,innerOpts={}){
  ...
  const {T0,TF,TR}=fixedTemps; const rho=1.365,cp=0.24;
  ...
  const F=(x)=>{
    ...
    else{const T3=T2+loads.QEV/denom;const MR=loads.QR/(rho*cp*(TR-T3)*PR);const MF=fan.totalAirflow-MR;F1=loads.QF - MF*rho*cp*(TF-T2)*PR;}
    return [F1,F2];
  };
  ...
  const sr=fPR*(condConfig.K_side/10)*(TC-T0), br=fPR*(condConfig.K_back/10)*(TC-T0);
  const loads=heatLoads(geom,{T0,TF,TR,T2:fT2,TC,PR:fPR},elec,{side:sr,back:br},fan.totalAirflow,fan.inputPower_W);
  const comp=compressorStateCorrected(TC,TE,refrig,compParams,subcool);
...
function solveSystem(config){
  ...
  const T0=fixedTemps.T0; let TC=TC0,totalInner=0;
  const TE=-25.27;
  for(let i=0;i<maxIterOuter;i++){
    ...
    const F3=calcQCout(TC,T0,fixedTemps.TF,fixedTemps.TR,areas)-calcQCin(TC,TE,refrigerant,compParams,subcool,dischargeTemp);
    if(Math.abs(F3)<tolOuter) return {TC,T2:inner.T2,PR:inner.PR,converged:true,outerIterations:i+1,innerTotalIterations:totalInner,heatLoads:inner.heatLoads,compressor:inner.compressor};
    ...
    const dF3=((calcQCout(TC+DH,T0,fixedTemps.TF,fixedTemps.TR,areas)-calcQCin(TC+DH,TE,refrigerant,compParams,subcool,dischargeTemp))-F3)/DH;
    if(Math.abs(dF3)<1e-9) return {TC,T2:NaN,PR:NaN,converged:false,error:'Zero derivative'};
...
// ── SJ-540 config ─────────────────────────────────────────────────────
const geom={H:1680,W:800,D:630,Hf:550,Hr:1130,Hb:260,Db1:210,Db2:230,doorGap:10,packingPos:15,tFtop:59.4,tFleft:59.4,tFright:59.4,tFbottom:70,tFdoor:59.4,tEvaBack:60,tRtop:70,tRleft:40,tRright:40,tRback:60,tRbottom1:40,tRbottom2:40,tRbottom3:40,tRdoor:40};
const compParams={...SJ54H_COMPONENTS.compressor};
const condConfig={K_side:5.395,K_back:4.17,backCondenserEfficiency:0.7,k_RFront1:0.3405,k_RFront2:0.03322,k_FRPartition1:0.1984,k_FRPartition2:0.1219,k_FFront1:0.3395,k_FFront2:0.0344};

const config={geom,compParams,condenserConfig:condConfig,refrigerant:'R-600a',subcool:10,dischargeTemp:60,fixedTemps:{T0:30,TF:-18,TR:3},fan:{totalAirflow:59.5,inputPower_W:2.1},elec:{defrostHeater_W:140,defrostOn_min:0,pwbOn_W:2,pwbOff_W:1,timerPeriod_h:10.5},TC0:45,DH:0.001,tolOuter:0.001,maxIterOuter:50,innerOpts:{dx:0.001,tol:1e-4,maxIter:100}};

console.log('=== SJ-540 SOLVER (correct wall temps & area) ===');
