...
// ------------------------------------------------------------------
// Inner solver (2×2 Newton) with DYNAMIC condenser rises
// ------------------------------------------------------------------
...
function solveInner(TC, geom, compParams, refrigerant, subcool, fixedTemps, fan, electrical, condenserConfig, innerOpts = {}) {
  ...
  const F = (x) => {
    ...
    // ---- DYNAMIC condenser rises (average over cycle) ----
    ...
    if (isNaN(comp.coolingCapacity)) {
      throw new Error(`compressorState returned NaN`);
    }
  ...
  // Final evaluation with converged values
  const finalT2 = result.x[0];
  const finalPR = result.x[1];
  const finalSideRise = finalPR * (condenserConfig.K_side / 10) * (TC - T0);
  const finalBackRise = finalPR * (condenserConfig.K_back / 10) * (TC - T0);
  const finalTemps = { T0, TF, TR, T2: finalT2, TC, PR: finalPR, TE };
  const finalHeatLoads = calcHeatLoads(geom, finalTemps, electrical, { side: finalSideRise, back: finalBackRise }, fan.totalAirflow, geom.evap, fan.inputPower_W);
...
// ------------------------------------------------------------------
// Outer solver with dynamic condenser rises and step limiting
// ------------------------------------------------------------------
function solveThermalSystemLogged(config) {
  ...
  for (let iter = 0; iter < maxIterOuter; iter++) {
    ...
    const dTC = F3 / dF3dTC;
    TC -= Math.max(-2, Math.min(2, dTC));  // limit step to ±2 °C
  }
  return { TC, T2: NaN, PR: NaN, converged: false, error: 'Outer loop max iterations' };
}

// ------------------------------------------------------------------
// Main: SJ-540 configuration
// ------------------------------------------------------------------
...
console.log('=== SJ-540 SOLVER (all fixes applied) ===');
