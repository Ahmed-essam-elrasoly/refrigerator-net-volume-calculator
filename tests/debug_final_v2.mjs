// tests/debug_final_v3.mjs – uses exact TE from converged Excel (-25.27°C)
...
// ── Corrected heat load (Excel replica) ─────────────────────────────
const PC = PHYSICAL_CONSTANTS;
function calcK(thk, lam) { const m=thk/1000; return 1/(1/PC.surfaceCoefficients.outside+1/PC.surfaceCoefficients.inside+m/lam); }
const kCache = new Map();
function kU(t) { if(!kCache.has(t)) kCache.set(t, calcK(t, PC.insulation.urethane)); return kCache.get(t); }
...
function heatLoads(geom, temps, elec, rises, fanFlow, fanW) {
  ...
  const {T0,TF,TR,T2,TC,PR}=temps; // TE is not used here
  ...
  let QF = kU(tFtop)*AFtop*(T0-TF) + kU(tFleft)*AFleft*(T_side-TF) + kU(tFright)*AFleft*(T_side-TF)
           + kU(tFbottom)*AFbottom*(TR-TF) + kU(tFdoor)*AFdoor*(T0-TF) + PC.insulation.packing*AFpackin*(T0-TF);
  ...
  let QR = kU(tRtop)*ARtop*(TF-TR) + kU(tRleft)*ARleft*(T_side-TR) + kU(tRright)*ARleft*(T_side-TR)
           + kU(tRback)*ARback*(T_back-TR) + kU(tRbottom1)*ARbottom1*(T_back-TR) + kU(tRbottom2)*ARbottom2*(T_back-TR)
           + kU(tRbottom3)*ARbottom3*(T0-TR) + kU(tRdoor)*ARdoor*(T0-TR) + PC.insulation.packing*ARpackin*(T0-TR);
  const DPCON_R = (0.0546*(TC-TF)-0.0491*(T0-TF))*PR*(Hr*2+W)/1000;
  QR += DPCON_R;
  const A_evaBack = (W-(tFleft+tFright)/2)*(Hf-(tFtop+tFbottom)/2)/1e6;
  const QEV_cond = kU(tEvaBack)*A_evaBack*(T_back-T2);
...
// ── Inner solver (uses fixed TE = -25.27) ────────────────────────────
function solveInner(TC, geom, compParams, refrig, subcool, fixedTemps, fan, elec, condConfig, innerOpts={}){
  ...
  const F=(x)=>{
    ...
    // Use Excel's exact TE (-25.27) for compressor
    const comp=compressorState(TC, -25.27, refrig, compParams, subcool);
...
function solveSystem(config){
  ...
  for(let iter=0;iter<maxIterOuter;iter++){
    ...
    const QCin=calcQCin(TC,-25.27,refrigerant,compParams,subcool,dischargeTemp);
    const F3=QCout-QCin;
    ...
    const dF3dTC=((calcQCout(TC+DH,T0,fixedTemps.TF,fixedTemps.TR,areas)-calcQCin(TC+DH,-25.27,refrigerant,compParams,subcool,dischargeTemp))-F3)/DH;
...
const config={
  ...
  fixedTemps:{T0:30,TF:-18,TR:3},   // TE not used, fixed to -25.27 in code
...
console.log('=== SJ-540 SOLVER (TE = -25.27°C) ===');
