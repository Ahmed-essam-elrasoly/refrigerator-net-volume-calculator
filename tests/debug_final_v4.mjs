// tests/debug_final_v4.mjs – exact Excel replica (TE, suction, liquid enthalpies)
...
// Temperature-dependent urethane conductivity (exact Excel formula)
function lambdaUrethane(T_inside, T_outside) {
  const T_avg = (T_inside + T_outside) / 2;
  return 0.0138 + 0.00011 * T_avg;   // kcal/m·h·°C
}

function calcK(thk_mm, T_inside, T_outside) {
  const lam = lambdaUrethane(T_inside, T_outside);
  const m = thk_mm / 1000;
  return 1 / (1/PC.surfaceCoefficients.outside + 1/PC.surfaceCoefficients.inside + m/lam);
}

// Updated K-value function – requires temps, so we remove the cache and calculate inline
function kUrethane(thk_mm, T_in, T_out) {
  return calcK(thk_mm, T_in, T_out);
}
...
function heatLoads(geom, temps, elec, rises, fanFlow, fanW) {
  ...
  const T_side = T0 + rises.side, T_back = T0 + rises.back;
  ...
  let QF = kUrethane(tFtop, TF, T0)*AFtop*(T0-TF) + kUrethane(tFleft, TF, T0)*AFleft*(T_side-TF) + kUrethane(tFright, TF, T0)*AFleft*(T_side-TF)
           + kUrethane(tFbottom, TF, T0)*AFbottom*(TR-TF) + kUrethane(tFdoor, TF, T0)*AFdoor*(T0-TF) + PC.insulation.packing*AFpackin*(T0-TF);
  ...
  let QR = kUrethane(tRtop, TF, T0)*ARtop*(TF-TR) + kUrethane(tRleft, TF, T0)*ARleft*(T_side-TR) + kUrethane(tRright, TF, T0)*ARleft*(T_side-TR)
           + kUrethane(tRback, TF, T0)*ARback*(T_back-TR) + kUrethane(tRbottom1, TF, T0)*ARbottom1*(T_back-TR) + kUrethane(tRbottom2, TF, T0)*ARbottom2*(T_back-TR)
           + kUrethane(tRbottom3, TF, T0)*ARbottom3*(T0-TR) + kUrethane(tRdoor, TF, T0)*ARdoor*(T0-TR) + PC.insulation.packing*ARpackin*(T0-TR);
  const DPCON_R = (0.0546*(TC-TF)-0.0491*(T0-TF))*PR*(Hr*2+W)/1000;
  QR += DPCON_R;
  const A_evaBack = (W-(tFleft+tFright)/2)*(Hf-(tFtop+tFbottom)/2)/1e6;
  const QEV_cond = kUrethane(tEvaBack, T2, T_back)*A_evaBack*(T_back-T2);
...
// ── Compressor (corrected: cooling uses evaporator outlet enthalpy) ───
function compressorStateCorrected(TC, TE, refrigerantName, compParams, subcool) {
  ...
  const T_suc = compParams.T_suction;   // 32.2
  ...
  const h_evap_out = rf.vaporEnthalpy(TE, Pe);   // evaporator outlet (saturated vapor)
...
// ── Inner solver (dynamic TE from Excel converged value for now) ───────
function solveInner(TC, geom, compParams, refrig, subcool, fixedTemps, fan, elec, condConfig, innerOpts={}){
  ...
  const TE = -25.27;   // Excel converged TE
  const F=(x)=>{
    const T2=x[0], PR=x[1];
...
console.log('=== SJ-540 SOLVER (TE fixed, evaporator outlet enthalpy for cooling) ===');
