// tests/debug_final_v7.mjs – corrected refrigerator side/back areas
...
function lambdaUrethane(T_inside, T_outside) {
  ...
  return 0.0165 + 0.00011 * (T_avg - 25);   // exact Excel formula
}
...
function heatLoads(geom, temps, elec, rises, fanFlow, fanW) {
  ...
  // ── CORRECTED refrigerator areas (using tRbottom3, not tRtop) ──
  const ARtop    = (W - (tRleft+tRright)/2) * (D - tRback/2) / 1e6;
  const ARleft   = ((Hr - (tRbottom1 + tRbottom3)/2) * (D - tRback/2) - (Db1+Db2)*Hb/2) / 1e6;
  const ARback   = (Hr - (tRbottom1 + tRbottom3)/2 - Hb) * (W - (tRleft+tRright)/2) / 1e6;
  ...
  let QR = kInterior(tRtop,TF,TR)*ARtop*(TF-TR)
         ...
         + kExterior(tRbottom1,TR,T_back)*ARbottom1*(T_back-TR)
...
// ── Compressor (evaporator outlet enthalpy) ───────────────────────────
...
console.log('=== SJ-540 SOLVER (corrected side/back areas) ===');
