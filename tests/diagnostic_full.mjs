// tests/diagnostic_full.mjs – term‑by‑term comparison against Excel for SJ‑540 & PV73K
...
function compLine(label, ourVal, excelVal, unit = '') {
  const diff = typeof ourVal === 'number' ? ourVal - excelVal : 0;
  const flag = Math.abs(diff) > 0.015 ? ' <--' : '';
  return `${label.padEnd(18)} ${ourVal.toFixed(4).padStart(8)} ${excelVal.toFixed(4).padStart(8)}  Δ=${diff.toFixed(4)}${flag}`;
}

// ── SJ‑540 (top‑freezer) geometry & operating point ──
const geom54 = {
...
// ── PV73K (bottom‑freezer) geometry & operating point ──
const geom73 = {
...
// SJ‑540 (from earlier diagnostic that matched Excel)
const excel54 = {
  freezer: {
...
// PV73K (from refined SIZE sheet)
const excel73 = {
  refrigerator: {  // top compartment, "R" labels in Excel
    top: 2.7188, left: 3.9693, right: 3.9693, bottom: -3.7473, back: 3.2156,
    door: 4.1992, packin: 2.7458, dpcon1: 4.7592, dpcon2: 4.7895,
  },
  freezer: {       // bottom compartment, "F" labels in Excel
...
// ── Wall temperature calculation (same as in heatLoad.js) ──
function computeWallTemps(geom, op, pipePitch, backEff) {
  const { H, W, D, Hb, Db1, Db2 } = geom;
  const { T0, PR, TC } = op;
  ...
  const S_back = W * (H - Hb) / 1e6 * backEff;
  ...
  return { T_wallSide, T_wallBack, T_comp, S_side, S_back, K_side, K_back };
}

// ── Diagnostic for a top‑freezer (SJ‑540) ──
function diagTopFreezer(geom, op, pipePitch, backEff, fan, excel) {
  const { T0, TF, TR, T2, TC, PR, TE } = op;
  const walls = computeWallTemps(geom, op, pipePitch, backEff);
  const T_side = walls.T_wallSide;
  const T_back = walls.T_wallBack;
  const T_comp = walls.T_comp;
  const T_wallBack_comp = (T_comp - T0) * PR + T0; // Cab Bottom

  let out = '';

  // ── Freezer ──
  out += '--- Freezer ---\n';
  const W = geom.W, D = geom.D, Hf = geom.Hf;
  ...
  // F RIGHT (same as left)
  out += `F RIGHT     S=${AFleft.toFixed(4)} B=${(K_Fleft*AFleft).toFixed(4)} K=${K_Fleft.toFixed(4)} Ti=${TF} To=${T_side.toFixed(2)} Q=${Q_Fleft.toFixed(4)} (Excel ${excel.freezer.right})\n`;
  ...
  out += `R RIGHT     S=${ARleft.toFixed(4)} B=${(K_Rleft*ARleft).toFixed(4)} K=${K_Rleft.toFixed(4)} Ti=${TR} To=${T_side.toFixed(2)} Q=${Q_Rleft.toFixed(4)} (Excel ${excel.refrigerator.right})\n`;
...
// ── Diagnostic for a bottom‑freezer (PV73K) ──
function diagBottomFreezer(geom, op, pipePitch, backEff, fan, excel) {
  const { T0, TF, TR, T2, TC, PR, TE } = op;
  const walls = computeWallTemps(geom, op, pipePitch, backEff);
  const T_side = walls.T_wallSide;
  const T_back = walls.T_wallBack;
  const T_comp = walls.T_comp;
  // For bottom‑freezer: freezer (bottom) has machine compartment cut‑out, refrigerator (top) full height
  // We'll compute terms exactly as in the Excel SIZE sheet: "R" labels = refrigerator (top), "F" labels = freezer (bottom)
  let out = '';

  // ── Refrigerator (top, R labels) ──
  out += '--- Refrigerator (top, R labels) ---\n';
  const W = geom.W, D = geom.D, Hr = geom.Hr;
  const { tRtop, tRleft, tRright, tRback, tRdoor, tRfloor } = geom; // tRfloor = partition thickness (32)
  ...
  // R LEFT / RIGHT (no cut-out)
  const rSideHeight = Hr - (tRtop + tRfloor)/2;
  const ARleft = (rSideHeight * (D - tRback/2)) / 1e6;
  const K_Rleft = kExterior(tRleft, TR, T_side);
  const Q_Rleft = K_Rleft * ARleft * (T_side - TR);
  const Q_Rright = K_Rleft * ARleft * (T_side - TR); // same as left
  out += `R LEFT      S=${ARleft.toFixed(4)} B=${(K_Rleft*ARleft).toFixed(4)} K=${K_Rleft.toFixed(4)} Ti=${TR} To=${T_side.toFixed(2)} Q=${Q_Rleft.toFixed(4)} (Excel ${excel.refrigerator.left})\n`;
  out += `R RIGHT     S=${ARleft.toFixed(4)} B=${(K_Rleft*ARleft).toFixed(4)} K=${K_Rleft.toFixed(4)} Ti=${TR} To=${T_side.toFixed(2)} Q=${Q_Rleft.toFixed(4)} (Excel ${excel.refrigerator.right})\n`;

  // R BOTTOM (partition to freezer)
  const ARbottom = (W - (tRleft + tRright)/2) * (D - tRback/2) / 1e6;
  const K_Rbottom = kInterior(tRfloor, TF, TR); // tRfloor = partition
  ...
  // R BACK (no cut-out? Actually Excel R BACK uses T_back? In sheet it's T_back=26.71)
  ...
  // R DPCON1 & DPCON2 (partition losses for refrigerator)
  ...
  // ── Freezer (bottom, F labels) ──
  out += '--- Freezer (bottom, F labels) ---\n';
  const Hf = geom.Hf, Hb = geom.Hb, Db1 = geom.Db1, Db2 = geom.Db2;
  const { tFtop, tFleft, tFright, tFdoor, tFfloor1, tFfloor2, tFfloor3, tEvaBack, tFback } = geom;

  // F TOP (partition, so interior K)
  const AFtop = (W - (tFleft + tFright)/2) * (D - tEvaBack/2) / 1e6;
  ...
  // F BOTTOM1 (machine comp top)
  const AFbottom1 = (W - (tFleft + tFright)/2) * Db1 / 1e6;
  const K_Fb1 = kExterior(tFfloor1, TF, T_comp);
  const Q_Fb1 = K_Fb1 * AFbottom1 * (T_comp - TF);
  out += `F BOTTOM1   S=${AFbottom1.toFixed(4)} B=${(K_Fb1*AFbottom1).toFixed(4)} K=${K_Fb1.toFixed(4)} Ti=${TF} To=${T_comp.toFixed(2)} Q=${Q_Fb1.toFixed(4)} (Excel ${excel.freezer.bottom1})\n`;

  // F BOTTOM2 (slope)
  const AFbottom2 = (W - (tFleft + tFright)/2) * Math.sqrt(Hb*Hb + (Db2 - Db1)**2) / 1e6;
  const K_Fb2 = kExterior(tFfloor2, TF, T_comp);
  const Q_Fb2 = K_Fb2 * AFbottom2 * (T_comp - TF);
  out += `F BOTTOM2   S=${AFbottom2.toFixed(4)} B=${(K_Fb2*AFbottom2).toFixed(4)} K=${K_Fb2.toFixed(4)} Ti=${TF} To=${T_comp.toFixed(2)} Q=${Q_Fb2.toFixed(4)} (Excel ${excel.freezer.bottom2})\n`;

  // F BOTTOM3 (ambient floor)
  const AFbottom3 = (W - (tFleft + tFright)/2) * Db2 / 1e6;
  ...
  // F DPCON (freezer front partition loss)
  ...
  // ── Evaporator (freezer back, with machine compartment cut-out) ──
  out += '--- Evaporator ---\n';
  const A_evaBack = (W - (tFleft + tFright)/2) * (Hf - Hb - (tFtop + tFfloor1)/2) / 1e6; // bottom‑freezer cut-out
...
// ── Main: run both diagnostics and write to file ──
const out = [];
out.push('============ SJ‑540 Top‑Freezer ============');
out.push(diagTopFreezer(geom54, op54, pipe54, backEff54, fan54, excel54));
out.push('============ PV73K Bottom‑Freezer ============');
out.push(diagBottomFreezer(geom73, op73, pipe73, backEff73, fan73, excel73));
