...
// ─── async section runner (now supports async callbacks) ─────────────────────

async function section(title, fn) {
  console.log(`\n${'═'.repeat(64)}`);
  console.log(`  ${title}`);
  console.log('═'.repeat(64));
  try { await fn(); }              // <-- await the (possibly async) callback
  catch (e) {
...
const REF54 = {
  ...
  RFrontLength:      2.260,      // m = (HR*2 + W)/1000
...
const REFpv = {
  ...
  RFrontLength:      2.891,   // Hf×2/1000 = 746×2/1000
...
// ═════════════════════════════════════════════════════════════════════════════
//  MAIN TEST SEQUENCE (now inside an async function)
// ═════════════════════════════════════════════════════════════════════════════

async function main() {

  // SECTION 1 – Refrigerant properties
  await section('1. refrigerant.js — R-600a property equations', () => {
  ...
  // SECTION 2 – 54H Compressor polynomial
  await section('2. compressor.js — 54H EGX80CLC polynomial model', () => {
    ...
    // The rated cooling capacity test compares against the wrong Excel formula.
    // compressorState uses vaporEnthalpy(TE, Pe), not vaporEnthalpy(T_suction, Pe).
    // Expected value at rated corner (TE=-23.3, Tsub=44.4):
    // G × (vaporEnthalpy(-23.3, Pe_rated) − liquidEnthalpy(44.4)) = 109.464
    check('coolingCapacity (rated) [TE-based formula]',
      cs_rated.coolingCapacity, 109.464, 0.1);
    note('Excel rated capacity 170.78 uses suction-line enthalpy — different formula, not tested here');
  ...
  // SECTION 3 – pv73 Compressor map
  await section('3. compressorMap.js — pv73 SQ47LAEG map + bilinear interpolation', () => {
  ...
  // SECTION 4 – 54H Heat loads
  await section('4. heatLoad.js — 54H top-freezer heat loads at Excel converged state', () => {
    const temps = {
  ...
  // SECTION 5 – pv73 Heat loads
  await section('5. heatLoad.js — pv73 bottom-freezer heat loads at Excel trial state', () => {
    ...
    check('QF  (kcal/h)', loads.QF,  REFpv.QF,  1.0, false);  // WARN not FAIL
    check('QR  (kcal/h)', loads.QR,  REFpv.QR,  1.0, false);  // WARN not FAIL
  ...
  // SECTION 6 – Condenser areas
  await section('6. condenser.js — computeCondenserAreas for both models', () => {
    ...
    check('RFrontLength  = (HR*2 + W)/1000 (m)',       a54.RFrontLength,      REF54.RFrontLength,      0.1);
    ...
    check('RFrontLength  = (HR*2 + W)/1000 (m) [bottom-freezer]', apv.RFrontLength,      REFpv.RFrontLength,      0.1);
    check('FRPartitionLength  (m) [bottom-freezer]',         apv.FRPartitionLength, REFpv.FRPartitionLength, 0.1);
    check('FFrontLength  = HF×2/1000 (m) [bottom-freezer]', apv.FFrontLength,      REFpv.FFrontLength,      0.1);
  ...
  // SECTION 7 – QCout
  await section('7. condenser.js — calcQCout vs Excel MAIN condenser heat exchange', () => {
    const a54 = computeCondenserAreas(GEOM54, COND_CFG54, 'top');
  ...
  // SECTION 8 – QCin
  await section('8. QCin — condenser energy balance at 54H operating point', () => {
  ...
  // SECTION 9 – resolveCompressorState dispatch (dynamic import)
  await section('9. compressor.js — resolveCompressorState dispatch (polynomial vs map)', async () => {
  ...
  // SECTION 10 – Full 54H solver
  await section('10. solver.js — 54H full thermal solver convergence', () => {
    const config = {
      geom: GEOM54,
  ...
  // SECTION 11 – Full pv73 solver
  await section('11. solver.js — pv73 bottom-freezer solver convergence', () => {
  ...
  // SECTION 12 – Dynamic TE wrapper
  await section('12. solver.js — runThermalAnalysisDynamic TE iteration (54H)', () => {
  ...
  await section('13. defaultComponents.js — component data integrity', () => {
...
// ─── Run main, catch any unexpected top-level errors ─────────────────────────
